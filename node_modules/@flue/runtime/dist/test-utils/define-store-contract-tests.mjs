import { g as PersistedFormatVersionError } from "../errors-CsDcT_C4.mjs";
...
async function admitDispatchReady(store, input) {
	...
	return {
		kind: "submission",
		submission: await store.markSubmissionCanonicalReady(admission.submission.submissionId) ?? admission.submission
	};
}
	...
	describe(label, () => {
		...
		describe("dispatch admission", () => {
			...
			it("returns conflict when one submission id is reused with another payload", async () => {
				...
				expect(await store.admitDispatch(dispatchInput({ message: {
					kind: "signal",
					type: "test.event",
					body: "Different"
			...
			it("round-trips a dispatched user message with attachments", async () => {
				const store = await create();
				const input = dispatchInput({ message: {
					kind: "user",
				...
				expect((await store.getSubmission(input.submissionId))?.input).toMatchObject({ message: input.message });
			});
			it("returns conflict when one submission id is replayed with different attachment bytes", async () => {
				const store = await create();
				const attachmentMessage = (data) => ({
					kind: "user",
					body: "Hello",
					attachments: [{
						type: "image",
				...
				expect(await store.admitDispatch(dispatchInput({ message: attachmentMessage("second-image") }))).toEqual({ kind: "conflict" });
			});
			it("chunks dispatched attachment bytes out of the stored payload and hydrates them on read", async () => {
				...
				expect((await store.getSubmission(input.submissionId))?.input.message).toEqual(input.message);
			});
		});
		describe("direct admission", () => {
			it("round-trips direct submission images", async () => {
				const store = await create();
				const input = directInput({ message: {
					kind: "user",
					body: "Hello",
					attachments: [{
						type: "image",
			...
			it("rejects replay when a direct submission image has different bytes", async () => {
				const store = await create();
				await admitDirectReady(store, directInput({ message: {
					kind: "user",
					body: "Hello",
					attachments: [{
						type: "image",
				...
				await expect(admitDirectReady(store, directInput({ message: {
					kind: "user",
					body: "Hello",
					attachments: [{
						type: "image",
		...
		describe("canonical readiness", () => {
			...
			it("lists unready queued submissions in admission order", async () => {
				...
				const third = await store.admitDirect(directInput({
					submissionId: "direct-2",
					id: "agent-2"
				}));
				expect(await store.listUnreadySubmissions()).toEqual([expect.objectContaining({ submissionId: first.kind === "submission" ? first.submission.submissionId : "" }), expect.objectContaining({ submissionId: third.submissionId })]);
			});
		...
		describe("submission ordering", () => {
			...
			it("lists queued dispatches in admission order and selects one runnable head per session", async () => {
				...
				expect(await store.listRunnableSubmissions()).toEqual([expect.objectContaining({ submissionId: "dispatch-1" }), expect.objectContaining({ submissionId: "dispatch-3" })]);
			});
			it("keeps agents with the same instance id in independent sessions", async () => {
				...
				expect(await store.listRunnableSubmissions()).toEqual([expect.objectContaining({ submissionId: "dispatch-1" }), expect.objectContaining({ submissionId: "dispatch-2" })]);
				expect(await store.claimSubmission(claim("dispatch-1", "attempt-1"))).toMatchObject({ status: "running" });
				expect(await store.claimSubmission(claim("dispatch-2", "attempt-2"))).toMatchObject({ status: "running" });
			});
		});
		...
		describe("lifecycle transitions", () => {
			...
			it("requeues a running attempt on ownership alone — the durability stamp is not a gate", async () => {
				...
				expect(await store.requeueSubmission(attempt("requeue-owned", "attempt-imposter"))).toBe(false);
				expect(await store.requeueSubmission(attempt("requeue-owned", "attempt-owned"))).toBe(true);
				expect(await store.requeueSubmission(attempt("requeue-stamped", "attempt-stamped"))).toBe(true);
				expect(await store.getSubmission("requeue-owned")).toMatchObject({ status: "queued" });
			...
			it("ignores stale-attempt settlement and keeps the first owning terminal state", async () => {
				...
				await store.failSubmission(attempt("dispatch-1", "attempt-1"), /* @__PURE__ */ new Error("first failure"));
				await store.completeSubmission(attempt("dispatch-1", "attempt-1"));
				await store.failSubmission(attempt("dispatch-1", "attempt-1"), /* @__PURE__ */ new Error("later failure"));
				expect(await store.getSubmission("dispatch-1")).toMatchObject({
					status: "settled",
					error: "first failure"
				});
		...
		describe("session abort requests", () => {
			...
			it("stamps the running head and every queued submission in the session at once", async () => {
				...
				const sessionKey = admitted.kind === "submission" ? admitted.submission.sessionKey : "";
				const affected = await store.requestSessionAbort(sessionKey);
				expect(new Set(affected)).toEqual(/* @__PURE__ */ new Set([
					"dispatch-1",
					"dispatch-2",
					"dispatch-3"
				]));
			...
			it("scopes the abort to one agent when another agent shares the instance id", async () => {
				const store = await create();
				const alpha = await admitDispatchReady(store, dispatchInput({
					agent: "alpha",
					id: "shared"
				}));
				await admitDispatchReady(store, dispatchInput({
				...
				expect(beta?.abortRequestedAt ?? void 0).toBeUndefined();
			});
		...
		describe("settleQueuedSubmission()", () => {
			it("settles an unready queued submission as failed with the error message", async () => {
				const store = await create();
				await store.admitDispatch(dispatchInput());
				expect(await store.settleQueuedSubmission("dispatch-1", "failed", /* @__PURE__ */ new Error("materialization permanently failing"))).toBe(true);
				const settled = await store.getSubmission("dispatch-1");
			...
			it("settles an abort-flagged queued submission as aborted", async () => {
				...
				const sessionKey = admitted.kind === "submission" ? admitted.submission.sessionKey : "";
				await store.requestSessionAbort(sessionKey);
				expect(await store.settleQueuedSubmission("dispatch-1", "aborted", /* @__PURE__ */ new Error("Submission was aborted."))).toBe(true);
				expect(await store.getSubmission("dispatch-1")).toMatchObject({
					status: "settled",
			...
			it("also settles a canonical-ready queued submission — queued is the only gate", async () => {
				const store = await create();
				await admitDispatchReady(store, dispatchInput());
				expect(await store.settleQueuedSubmission("dispatch-1", "aborted", /* @__PURE__ */ new Error("stop"))).toBe(true);
				expect(await store.getSubmission("dispatch-1")).toMatchObject({ status: "settled" });
			});
			it("returns false against a racing claim — the first terminal state wins both ways", async () => {
				...
				await admitDispatchReady(store, dispatchInput());
				await store.claimSubmission(claim("dispatch-1", "attempt-1"));
				expect(await store.settleQueuedSubmission("dispatch-1", "failed", /* @__PURE__ */ new Error("too late"))).toBe(false);
				expect(await store.getSubmission("dispatch-1")).toMatchObject({
					status: "running",
					attemptId: "attempt-1"
				});
				...
				expect(await store.settleQueuedSubmission("dispatch-2", "failed", /* @__PURE__ */ new Error("gave up"))).toBe(true);
			...
			it("returns false on a settled submission and preserves the first terminal state", async () => {
				...
				await store.claimSubmission(claim("dispatch-1", "attempt-1"));
				await store.completeSubmission(attempt("dispatch-1", "attempt-1"));
				expect(await store.settleQueuedSubmission("dispatch-1", "failed", /* @__PURE__ */ new Error("late failure"))).toBe(false);
				const settled = await store.getSubmission("dispatch-1");
			...
			it("returns false on joining and joined rows", async () => {
				...
				expect(await store.settleQueuedSubmission("joined-1", "aborted", /* @__PURE__ */ new Error("stop"))).toBe(false);
				expect(await store.settleQueuedSubmission("joining-1", "aborted", /* @__PURE__ */ new Error("stop"))).toBe(false);
			...
			it("returns false for an unknown submission id", async () => {
				const store = await create();
				expect(await store.settleQueuedSubmission("nonexistent", "failed", /* @__PURE__ */ new Error("boom"))).toBe(false);
			});
		});
		describe("direct settlement obligation", () => {
			...
			it("reserves a canonical settlement only for the owning direct attempt", async () => {
				...
				expect(await store.reserveSubmissionSettlement(attempt("direct-1", "stale"), {
					recordId: record.id,
					record
				})).toBeNull();
			...
			it("replays an exact obligation and rejects a conflicting settlement payload", async () => {
				...
				expect(await store.reserveSubmissionSettlement(ref, {
					recordId: completed.id,
					record: completed
				})).toEqual(first);
				expect(await store.reserveSubmissionSettlement(ref, {
			...
			it("lists a pending dispatch settlement obligation", async () => {
				...
				expect(await store.listPendingSubmissionSettlements()).toContainEqual(expect.objectContaining({
					submissionId: "dispatch-1",
			...
			it("settles a dispatch through the same reserve/finalize outbox", async () => {
				...
				expect(await store.reserveSubmissionSettlement(attempt("dispatch-1", "stale"), {
					recordId: record.id,
					record
				})).toBeNull();
				const reserved = await store.reserveSubmissionSettlement(attempt("dispatch-1", "attempt-1"), {
					recordId: record.id,
					record
				});
				...
				expect(await store.getSubmission("dispatch-1")).toMatchObject({ status: "terminalizing" });
				expect(await store.finalizeSubmissionSettlement(attempt("dispatch-1", "attempt-1"), record.id)).toBe(true);
				const settled = await store.getSubmission("dispatch-1");
				expect(settled).toMatchObject({ status: "settled" });
				expect(settled?.error).toBeUndefined();
			});
			it("mirrors the failed settlement error onto the operational row", async () => {
				...
				expect(await store.finalizeSubmissionSettlement(attempt("dispatch-1", "attempt-1"), record.id, { errorMessage: "provider exploded: boom" })).toBe(true);
				expect(await store.getSubmission("dispatch-1")).toMatchObject({
					status: "settled",
		...
		describe("durability", () => {
			...
			it("sets attempt_count to 1 and applies system defaults at claim time", async () => {
				...
				expect(claimed.timeoutAt).toBeGreaterThanOrEqual(before + 36e5);
			});
			it("applies custom durability when input is marked applied", async () => {
				const store = await create();
				await admitDispatchReady(store, dispatchInput());
				const customTimeout = Date.now() + 216e5;
				await store.claimSubmission(claim("dispatch-1", "attempt-1"));
		...
		describe("listExpiredSubmissions()", () => {
			...
			it("returns empty when no submissions exist", async () => {
				const expired = await (await create()).listExpiredSubmissions();
				expect(expired).toHaveLength(0);
			});
		});
		describe("turn-boundary joins", () => {
			/** A running host with `count` queued dispatches behind it. */
			async function hostWithQueued(store, count) {
				await admitDispatchReady(store, dispatchInput({ submissionId: "host-1" }));
				await store.claimSubmission(claim("host-1", "attempt-1"));
				for (let index = 0; index < count; index++) await admitDispatchReady(store, dispatchInput({ submissionId: `queued-${index + 1}` }));
			...
			it("claims the queued dispatch prefix in admission order onto a running host", async () => {
				...
				expect(claimed.map((submission) => submission.submissionId)).toEqual(["queued-1", "queued-2"]);
			...
			it("claims nothing for a stale attempt or a host that is not running", async () => {
				...
				expect(await store.claimJoinableSubmissions(attempt("host-1", "stale-attempt"), "assistant")).toEqual([]);
				await store.completeSubmission(host);
				expect(await store.claimJoinableSubmissions(host, "assistant")).toEqual([]);
				expect(await store.getSubmission("queued-1")).toMatchObject({ status: "queued" });
			});
			...
			it("stops the prefix at the first non-joinable row, preserving admission order", async () => {
				...
				expect(await store.getSubmission("unready-mid")).toMatchObject({ status: "queued" });
				expect(await store.getSubmission("queued-behind")).toMatchObject({ status: "queued" });
			});
			it("reserves a joined direct delivery settlement under the host attempt", async () => {
				...
				expect(await store.reserveSubmissionSettlement(attempt("direct-joined", "stale-attempt"), {
					recordId: record.id,
					record
				})).toBeNull();
				const reserved = await store.reserveSubmissionSettlement(attempt("direct-joined", "attempt-1"), {
					recordId: record.id,
					record
				});
				expect(reserved).toMatchObject({
				...
				expect(await store.finalizeSubmissionSettlement(attempt("direct-joined", "attempt-1"), record.id)).toBe(true);
			...
			it("stops the prefix at an abort-requested delivery and at a different agent", async () => {
				...
				await store.requestSessionAbort("agent-session:[\"assistant\",\"agent-1\",\"default\",\"default\"]");
				await admitDispatchReady(store, dispatchInput({ submissionId: "queued-behind" }));
				expect(await store.claimJoinableSubmissions(host, "assistant")).toEqual([]);
			...
			it("finalizes a claimed join once and fences on the host attempt", async () => {
				...
				const host = await hostWithQueued(store, 1);
				await store.claimJoinableSubmissions(host, "assistant");
				expect(await store.finalizeJoinedSubmission(attempt("host-1", "stale-attempt"), "queued-1")).toBe(false);
			...
			it("reverts an unapplied join back to the queue", async () => {
				...
				const host = await hostWithQueued(store, 1);
				await store.claimJoinableSubmissions(host, "assistant");
				expect(await store.revertJoiningSubmission(attempt("host-1", "stale-attempt"), "queued-1")).toBe(false);
			...
			it("lists unsettled joins for the host in admission order", async () => {
				...
				expect(joined.map((submission) => [submission.submissionId, submission.status])).toEqual([["queued-1", "joined"], ["queued-2", "joining"]]);
			});
			it("joined deliveries settle with a completing host; joining stragglers revert", async () => {
				...
				expect(await store.getSubmission("queued-2")).toMatchObject({ status: "queued" });
			});
			it("joined deliveries share a failing host outcome", async () => {
				...
				await store.claimJoinableSubmissions(host, "assistant");
				await store.finalizeJoinedSubmission(host, "queued-1");
				await store.failSubmission(host, /* @__PURE__ */ new Error("host exploded"));
				expect(await store.getSubmission("queued-1")).toMatchObject({
					status: "settled",
					error: "host exploded"
				});
			...
			it("unsettled joins block later queued work and clear with the host", async () => {
				...
				expect(await store.hasUnsettledSubmissions()).toBe(true);
				await store.completeSubmission(host);
				expect((await store.listRunnableSubmissions()).map((submission) => submission.submissionId)).toEqual(["later"]);
			});
			it("requestSessionAbort stamps joining and joined deliveries", async () => {
				...
				await store.claimJoinableSubmissions(host, "assistant");
				await store.finalizeJoinedSubmission(host, "queued-1");
				const stamped = await store.requestSessionAbort("agent-session:[\"assistant\",\"agent-1\",\"default\",\"default\"]");
				expect(new Set(stamped)).toEqual(/* @__PURE__ */ new Set([
					"host-1",
					"queued-1",
					"queued-2"
				]));
...
function defineFormatVersionTests(formatVersion) {
	describe("format-version stamping", () => {
		async function withHandle(run) {
...
//#endregion
export { defineAttachmentStoreContractTests, defineConversationStreamStoreContractTests, defineStoreContractTests };
