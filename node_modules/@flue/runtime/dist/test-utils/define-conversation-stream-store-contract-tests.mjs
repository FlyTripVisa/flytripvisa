...
function userRecord(id, messageId, text = messageId) {
	return {
		v: 1,
		id,
		type: "user_message",
...
async function claimContractSubmission(submissionStore, submissionId, attemptId, agent = "echo") {
...
function defineConversationStreamStoreContractTests(label, backend) {
	describe(label, () => {
		...
		it("round-trips a batch larger than a backend cell-size cap", async () => {
			...
			const producer = await stream.acquireProducer("agents/echo/contract", "coordinator");
			const record = userRecord("record_large", "entry_large", "x".repeat(25e5));
			const result = await stream.append({
				path: "agents/echo/contract",
				producerId: producer.producerId,
				producerEpoch: producer.producerEpoch,
		...
		it("returns the original offset for an exact oversized retry", async () => {
			...
			const input = {
				...
				incarnation: producer.incarnation,
				producerSequence: 0,
				records: [userRecord("record_large", "entry_large", "x".repeat(25e5))]
			};
			expect(await stream.append(input)).toEqual(await stream.append(input));
			expect((await stream.read("agents/echo/contract")).batches).toHaveLength(1);
		});
		...
		it("withholds a fold checkpoint past the atOrBefore bound when supported", async () => {
			...
			expect(await stream.getFoldCheckpoint("agents/echo/contract", { atOrBefore: first.offset })).toBeNull();
			expect(await stream.getFoldCheckpoint("agents/echo/contract", { atOrBefore: second.offset })).toMatchObject({ offset: second.offset });
		});
	});
}
//#endregion
