...
var WorkersAITranscriptionModel = class {
	...
	async runNova3(audioBytes, mediaType, abortSignal) {
		...
		if (!this.config.credentials) throw new Error("Nova-3 transcription via REST requires credentials in the config. This is a bug — credentials should have been set by createWorkersAI.");
		return createRunBinary(this.config.credentials, this.modelId, audioBytes, mediaType, abortSignal);
	}
	normalizeNova3Response(raw, warnings) {
		const alt = (raw.results?.channels)?.[0]?.alternatives?.[0];
		const text = alt?.transcript ?? "";
		const segments = [];
		if (alt?.words && Array.isArray(alt.words)) for (const w of alt.words) segments.push({
			text: w.word ?? "",
			startSecond: w.start ?? 0,
			endSecond: w.end ?? 0
		});
...
*/
var WorkersAISpeechModel = class {
	get provider() {
		return this.config.provider;
	}
	constructor(modelId, settings, config) {
	...
	async doGenerate(options) {
		...
		if (options.outputFormat) warnings.push({
			details: "Workers AI TTS models do not support output format selection. Audio is returned as MP3.",
...
async function toUint8Array(output) {
	...
	if (typeof output === "object" && output !== null) {
		const obj = output;
		if (typeof obj.audio === "string") return Uint8Array.from(atob(obj.audio), (c) => c.charCodeAt(0));
	}
	throw new Error(`Unexpected output type from TTS model. Got ${Object.prototype.toString.call(output)}`);
}
//#endregion
...
*/
var WorkersAIRerankingModel = class {
	get provider() {
		return this.config.provider;
	}
	constructor(modelId, settings, config) {
...
*/
function documentsToContexts(documents, warnings) {
	if (documents.type === "text") return documents.values.map((text) => ({ text }));
	warnings.push({
...
//#endregion
//#region src/autorag-chat-language-model.ts
/**
...
function createWorkersAI(options) {
	if (!options.binding && !("accountId" in options && "apiKey" in options)) throw new Error("Invalid Workers AI configuration: you must provide either a binding (e.g. { binding: env.AI }) or credentials ({ accountId, apiKey }).");
	let binding;
	const isBinding = !!options.binding;
	if (options.binding) binding = options.binding;
	else {
		const { accountId, apiKey } = options;
		binding = { run: createRun({
			accountId,
			apiKey,
			fetch: options.fetch
		}) };
...
A gateway defaults to "default" but can be set via \`gateway\`. Otherwise use a Workers AI model id (e.g. "@cf/meta/llama-3.1-8b-instruct").`);
	...
	const createTranscriptionModel = (modelId, settings = {}) => new WorkersAITranscriptionModel(modelId, settings, {
		binding,
		gateway: options.gateway,
		provider: "workersai.transcription",
		isBinding,
		credentials: !isBinding && "accountId" in options ? {
			accountId: options.accountId,
			apiKey: options.apiKey
		} : void 0
	});
	const createSpeechModel = (modelId, settings = {}) => new WorkersAISpeechModel(modelId, settings, {
		binding,
		gateway: options.gateway,
		provider: "workersai.speech"
	});
	const createRerankingModel = (modelId, settings = {}) => new WorkersAIRerankingModel(modelId, settings, {
		binding,
		gateway: options.gateway,
...
* @see https://developers.cloudflare.com/ai-search/
*/
function createAISearch(options, providerName = "aisearch.chat") {
	const binding = options.binding;
	const createChatModel = (settings = {}) => new AISearchChatLanguageModel("@cf/meta/llama-3.3-70b-instruct-fp8-fast", settings, {
		binding,
		provider: providerName
	});
	const provider = (settings) => {
		if (new.target) throw new Error("The AISearch model function cannot be called with the new keyword.");
...
function createAutoRAG(options) {
	if (!autoRAGWarned) {
		autoRAGWarned = true;
		console.warn("[workers-ai-provider] createAutoRAG is deprecated. Use createAISearch instead. AutoRAG has been renamed to AI Search. See https://developers.cloudflare.com/ai-search/");
	}
